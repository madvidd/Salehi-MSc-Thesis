...
class MultimodalDecoder(nn.Module):
    ...
    def __init__(self, use_target_context, embed_dim=128, future_steps=60, k=6, variant='baseline') -> None:
        super().__init__()
        self.embed_dim = embed_dim
        self.future_steps = future_steps
        self.k = k
        attention_variant = 'qknorm' if variant in ('qknorm', 'combined') else 'baseline'
        self.attn_depth = 3
        dpr = [x.item() for x in torch.linspace(0, 0.2, self.attn_depth)]
        self.ctx_blks = nn.ModuleList((Block(dim=embed_dim, num_heads=8, mlp_ratio=4.0, qkv_bias=False, drop_path=dpr[i], cross_attn=True, kdim=embed_dim, vdim=embed_dim, attention_variant=attention_variant) for i in range(self.attn_depth)))
        if use_target_context:
            self.target_blks = nn.ModuleList((Block(dim=embed_dim, num_heads=8, mlp_ratio=4.0, qkv_bias=False, drop_path=dpr[i], cross_attn=True, kdim=embed_dim, vdim=embed_dim, attention_variant=attention_variant) for i in range(self.attn_depth)))
        self.pi = nn.Sequential(nn.Linear(embed_dim, embed_dim * 2), nn.ReLU(), nn.Linear(embed_dim * 2, 1))
        self.loc = nn.Sequential(nn.Linear(embed_dim, embed_dim * 2), nn.ReLU(), nn.Linear(embed_dim * 2, future_steps * 2))
