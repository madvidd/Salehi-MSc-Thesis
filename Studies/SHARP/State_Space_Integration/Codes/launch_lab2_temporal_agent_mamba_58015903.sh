...
SETUP="$REPO/Studies/SHARP/State_Space_Integration/Codes/setup_lab2_temporal_agent_mamba_ablation.py"
...
ROOT=$(cat "$BASE/Codes/LATEST_SHARP_AV2_TEMPORAL_AGENT_MAMBA80.txt")
RESULTS=$(cat "$BASE/Results/LATEST_SHARP_AV2_TEMPORAL_AGENT_MAMBA80.txt")
RUNNER="$ROOT/run_temporal_agent_mamba_4gpu.sh"
...
cd "$ROOT/Code"
echo "Starting temporal-agent Mamba training in this foreground terminal."
bash "$RUNNER"
STATUS=$?
