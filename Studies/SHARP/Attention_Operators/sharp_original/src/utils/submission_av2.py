...
class SubmissionAv2:
    ...
    def format_data(
        ...
        if self.ma:
            probability, indices = torch.sort(probability, dim=1, descending=True)
            global_trajectory = torch.gather(global_trajectory, 1, indices.unsqueeze(-1).unsqueeze(-1).expand(-1, -1, 60, 2))       
        ...
        for i, (scene_id, track_id) in enumerate(zip(scenario_ids, track_ids)):
            if self.ma:
                if scene_id not in self.challenge_submission.predictions.keys():
                    self.challenge_submission.predictions[scene_id] = [[1/6]*6, {}]
                self.challenge_submission.predictions[scene_id][1][track_id] = global_trajectory[i]
            else:
