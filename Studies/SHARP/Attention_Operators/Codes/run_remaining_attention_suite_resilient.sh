...
archive_variant() {
...

  {
    ...
    echo "epochs=80"
    echo "gpus=3"
    echo "batch_per_gpu=${BATCH_PER_GPU:-8}"
    echo "gradient_accumulation=${ACCUMULATE_GRAD_BATCHES:-1}"
    echo "effective_global_batch=$(( ${BATCH_PER_GPU:-8} * 3 * ${ACCUMULATE_GRAD_BATCHES:-1} ))"
    echo "seed=2333"
...
publish_archive() {
  ...
  if [[ "$result_kind" == completed ]]; then
    rel="Studies/SHARP/Attention_Operators/Main_Results/Attention_Experiments/$(basename "$archive")"
  else
    rel="Studies/SHARP/Attention_Operators/Main_Results/Attention_Experiments/Failed_Runs/$(basename "$archive")"
  fi
