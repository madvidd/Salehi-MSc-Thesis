...
from .attention_variants import VariantMultiheadAttention
...
class Block(nn.Module):
    def __init__(
        ...
        self.attn = VariantMultiheadAttention(
            dim,
            num_heads=num_heads,
...
class InterBlock(nn.Module):
    def __init__(
        ...
        self.drop_path2 = DropPath(drop_path) if drop_path > 0.0 else nn.Identity()
        if with_mask:
            self.id_bias_logit = nn.Parameter(torch.tensor(0.0))
            self.max_bias = 5.0
        return
    ...
    def forward(
        ...
        if type(mask) != type(None):
            ...
            bias_scalar = torch.sigmoid(self.id_bias_logit) * self.max_bias
            mask = mask * bias_scalar #[B, N_q, N_k]
            num_heads = self.attn.num_heads
            mask = mask.unsqueeze(1).repeat(1, num_heads, 1, 1)  # [B, heads, N_q, N_k]
            mask = mask.reshape(B * num_heads, N_q, N_k)        # [B*heads*N_q, N_k]
