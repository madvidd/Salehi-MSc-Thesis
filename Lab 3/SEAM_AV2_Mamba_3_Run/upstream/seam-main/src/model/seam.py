...
from .layers.mamba_layers import ResidualMambaBlock
...
class Seam_I(nn.Module):
    def __init__(
        ...
        self.mamba_config = {
            ...
            "d_conv": mamba_d_conv,
            "expand": mamba_expand,
            "future_depth": mamba_future_depth,
        }
        ...
        if self.variant == "mamba_agent_add":
            self.agent_history_mamba = ResidualMambaBlock(
                embed_dim=embed_dim,
                d_state=mamba_d_state,
                d_conv=mamba_d_conv,
                expand=mamba_expand,
    ...
    def forward(self, data):
        ...
        if self.variant == "mamba_agent_add":
            actor_feat = self.agent_history_mamba(
                actor_feat,
                valid_mask=(~kpm).to(actor_feat.dtype),
            )
...
class Seam(Seam_I):
    def __init__(self,
        ...
        self.decoder = MultimodalDecoder(
            ...
            mamba_d_state=self.mamba_config['d_state'],
            mamba_d_conv=self.mamba_config['d_conv'],
            mamba_expand=self.mamba_config['expand'],
            mamba_future_depth=self.mamba_config['future_depth'],
        )
