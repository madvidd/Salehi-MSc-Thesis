...
from .layers.controlled_ablation import (
...
class Seam_I(nn.Module):
    def __init__(
        ...
        if variant not in supported_variants:
            raise ValueError(
                f"Unknown SEAM variant {variant!r}; expected one of "
                f"{sorted(supported_variants)}"
            )

        self.variant = variant
        self.embed_dim = embed_dim
        self.attention_variant = "qknorm" if variant == "qknorm" else "baseline"
        ...
        if self.variant == "relative_geometry_bias":
    ...
    def forward(self, data):
        ...
        if self.variant == "uncertainty_target_context":
            actor_feat = actor_feat + self.uncertainty_target_context.zero_dependency(
                actor_feat
            )
        ...
        scene_centers = torch.cat(
            [data['x_centers'], data['lane_centers']], dim=1
        )
        scene_angles = torch.cat(
            [data['x_angles'][:, :, -1], data['lane_angles']], dim=1
        )
        x_angles = torch.stack(
            [torch.cos(scene_angles), torch.sin(scene_angles)], dim=-1
        )
        ...
        if "memory_dict" in data and data["memory_dict"] is not None and self.use_target_context:
            ...
            target_rel_centers = (
                scene_centers.unsqueeze(1).repeat(1, 6, 1, 1)
                - target_pos.unsqueeze(2)
            )
            target_rel_angles = (
                scene_angles.unsqueeze(1).repeat(1, 6, 1)
                - target_angle.unsqueeze(2)
            )
            target_angles = torch.stack(
                [torch.cos(target_rel_angles), torch.sin(target_rel_angles)], dim=-1
            )
            target_pos_feat = torch.cat(
                [target_rel_centers, target_angles], dim=-1
            )
            ...
            if self.variant == "uncertainty_target_context":
                ...
                if previous_logits is not None:
                    target_radius, target_gain = self.uncertainty_target_context(
                        previous_logits.detach()
                    )

            target_encoder = x_encoder_all.unsqueeze(1).expand_as(target_pos_embed) + target_pos_embed
            target_mask = (
                ~key_valid_mask_all.unsqueeze(1).expand(
                    B, 6, key_valid_mask_all.shape[1]
                )
            ) | (
                torch.norm(target_rel_centers, dim=-1)
                > target_radius.unsqueeze(-1)
            )
            ...
            compressed_target_encoder = (
                compressed_target_encoder.view(B, 6, -1, self.embed_dim)
                + target_center_embed.unsqueeze(2)
            )
            compressed_target_encoder = (
                compressed_target_encoder * target_gain[:, :, None, None]
            )
        ...
        if self.variant == "relative_geometry_bias":
            scene_attention_mask = self.relative_geometry_bias(
                scene_centers, scene_angles, key_valid_mask
            )
            scene_padding_mask = None
        for blk in self.blocks:
            x_encoder = blk(
                x_encoder,
                mask=scene_attention_mask,
