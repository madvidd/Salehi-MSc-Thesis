...
Curated result summaries are tracked under `Results/`; checkpoints, processed tensors, archives, and complete training logs remain excluded. Publication succeeds only after `Summary.md` and the compact `Terminal.txt` are verified on remote `main`.
