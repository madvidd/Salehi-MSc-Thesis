...
RESULTS_ROOT=$(tr -d '\r\n' < "$BASE/Results/LATEST_SEAM_AV2_20EPOCH_4TEST_MAXRES.txt" 2>/dev/null)
DATA_ROOT=$(tr -d '\r\n' < "$RESULTS_ROOT/DATA_ROOT.txt" 2>/dev/null)
ENV_DIR="$BASE/Codes/envs/seam_av2_mamba_torch211"
PYTHON="$ENV_DIR/bin/python"
GPU_IDS=${SEAM_GPU_IDS:-0,1,2}
GPU_COUNT=$(awk -F',' '{print NF}' <<< "$GPU_IDS")
CPU_COUNT=$(nproc)
DEFAULT_WORKERS=$((CPU_COUNT / GPU_COUNT - 2))
[ "$DEFAULT_WORKERS" -lt 4 ] && DEFAULT_WORKERS=4
[ "$DEFAULT_WORKERS" -gt 12 ] && DEFAULT_WORKERS=12
WORKERS_PER_RANK=${SEAM_WORKERS_PER_RANK:-$DEFAULT_WORKERS}
...
if [ "$GPU_COUNT" -ne 3 ]; then
  echo "ERROR: the max-resource profile requires exactly three visible GPUs."
  exit 1
fi
AVAILABLE_GPUS=$($PYTHON -c 'import torch; print(torch.cuda.device_count())' 2>/dev/null)
if [ "$AVAILABLE_GPUS" != "3" ]; then
  echo "ERROR: expected three CUDA devices after CUDA_VISIBLE_DEVICES=$GPU_IDS; found ${AVAILABLE_GPUS:-0}."
  exit 1
fi
...
echo "Resource profile: all three GPUs and $WORKERS_PER_RANK data workers per rank"
echo "Control: 20 epochs, effective global batch 48, AdamW, LR 1e-3 to 1e-5"
echo "Hardware: GPUs $GPU_IDS, batch 8/GPU, accumulation 2, CPU threads $CPU_COUNT"
...
for INDEX in 0 1 2 3; do
...

  {
    ...
    echo "resource_profile=max_resources_three_gpu"
    echo "effective_global_batch=48"
    echo "per_gpu_microbatch=8"
    echo "gradient_accumulation=2"
    echo "world_size=3"
    echo "data_workers_per_rank=$WORKERS_PER_RANK"
    echo "total_data_workers=$((WORKERS_PER_RANK * GPU_COUNT))"
