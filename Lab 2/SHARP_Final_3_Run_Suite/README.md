...
- Runs 2 and 3 retain that validated synchronous CUDA launch mode in production. This prevents asynchronous compute/collective overlap from deferring an illegal-access report to an unrelated later operation on the Lab 2 four-GPU runtime; it affects execution scheduling and throughput, not model structure, data order, FP32 precision, batch size, or optimization hyperparameters.
