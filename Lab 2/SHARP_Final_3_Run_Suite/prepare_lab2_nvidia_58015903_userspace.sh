...
if [ ! -s "$RUNFILE" ]; then
  echo "Downloading official NVIDIA $DRIVER_VERSION user-space archive."
  curl --fail --location --retry 3 --retry-delay 3 \
    "$ARCHIVE_BASE/$RUNFILE_NAME" --output "$RUNFILE"
else
  echo "Reusing $RUNFILE"
fi
if [ ! -s "$CHECKSUM_FILE" ]; then
  curl --fail --location --retry 3 --retry-delay 3 \
    "$ARCHIVE_BASE/$CHECKSUM_NAME" --output "$CHECKSUM_FILE"
fi
...
if [ -z "$EXTRACT_DIR" ]; then
  (cd "$COMPAT_ROOT" && sh "$RUNFILE" --extract-only)
  EXTRACT_DIR=$(find "$COMPAT_ROOT" -maxdepth 1 -type d \
    -name "NVIDIA-Linux-x86_64-$DRIVER_VERSION*" -print -quit)
fi

test -n "$EXTRACT_DIR"
CUDA_LIBRARY=$(find "$EXTRACT_DIR" -type f -name "libcuda.so.$DRIVER_VERSION" -print -quit)
NVML_LIBRARY=$(find "$EXTRACT_DIR" -type f -name "libnvidia-ml.so.$DRIVER_VERSION" -print -quit)
...
printf 'driver_version=%s\nlibrary_directory=%s\nbinary_directory=%s\narchive_sha256=%s\n' \
  "$DRIVER_VERSION" "$LIB_DIR" "$BIN_DIR" "$ACTUAL_SHA256" > "$MARKER"
