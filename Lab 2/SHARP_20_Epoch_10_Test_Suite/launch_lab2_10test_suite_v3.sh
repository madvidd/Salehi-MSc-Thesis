...
POINTER="$BASE/Codes/LATEST_SHARP_AV2_20EPOCH_10TEST_V3.txt"
...
if [ "$STATUS" -eq 0 ]; then
  ...
  if [ -s "$POINTER" ]; then
    ...
    if [ -x "$CANDIDATE/run_10_test_suite.sh" ] && \
       grep -q 'relative_geometry_prior' \
         "$CANDIDATE/Code/src/model/sharp.py" 2>/dev/null; then
      ROOT="$CANDIDATE"
      echo "Reusing resumable corrected experiment: $ROOT"
    fi
  fi
  ...
  if [ -z "$ROOT" ]; then
    cd "$PACKAGE" || STATUS=1
    if [ "$STATUS" -eq 0 ]; then
      "$PYTHON_BIN" setup_lab2_sharp_20epoch_10test_v3.py
      STATUS=$?
    fi
    if [ "$STATUS" -eq 0 ]; then
      ROOT=$(tr -d '\r\n' < "$POINTER")
