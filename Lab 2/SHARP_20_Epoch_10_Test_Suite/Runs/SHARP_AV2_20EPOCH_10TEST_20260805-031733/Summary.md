...
| Uncertainty-aware target context | Interrupted: CUDA illegal memory access | epoch index 0/19, 1% (56/6248) | - | 0 | - | - | - | - | - | - | - |
...
Completed variants: 4/10. No variant was active when this snapshot was verified. Test 5 stopped at batch 56 with an asynchronous illegal CUDA memory access reported on DDP rank 2; tests 6-10 had not started.

Post-publication verification: commit `16e52d5` updates the recovery path so only test 5 runs with synchronous CUDA execution. Tests 1-4 remain preserved, tests 6-10 retain normal asynchronous CUDA execution, and no training hyperparameter is changed.
