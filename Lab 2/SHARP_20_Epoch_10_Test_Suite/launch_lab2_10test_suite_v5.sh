...
POINTER="$BASE/Codes/LATEST_SHARP_AV2_20EPOCH_10TEST_V5.txt"
...
if [ "$STATUS" -eq 0 ]; then
  ...
  if [ -s "$POINTER" ]; then
    CANDIDATE=$(tr -d '\r\n' < "$POINTER")
    INDEX_COUNT=$(grep -c \
      'torch.arange(y_hat.shape\[0\], device=y_hat.device)' \
      "$CANDIDATE/Code/src/model/pl_modules.py" 2>/dev/null || true)
    if [ -x "$CANDIDATE/run_10_test_suite.sh" ] && \
       grep -q 'relative_geometry_prior' \
         "$CANDIDATE/Code/src/model/sharp.py" 2>/dev/null && \
       [ "$INDEX_COUNT" -eq 2 ]; then
      ROOT="$CANDIDATE"
      echo "Reusing resumable CUDA-index-corrected experiment: $ROOT"
    fi
  fi
  ...
  if [ -z "$ROOT" ]; then
    cd "$PACKAGE" || STATUS=1
    if [ "$STATUS" -eq 0 ]; then
      "$PYTHON_BIN" setup_lab2_sharp_20epoch_10test_v5.py
      STATUS=$?
    fi
    if [ "$STATUS" -eq 0 ]; then
      ROOT=$(tr -d '\r\n' < "$POINTER")
