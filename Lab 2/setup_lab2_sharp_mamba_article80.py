#!/usr/bin/env python3
"""Create an article-aligned 80-epoch SHARP + Mamba experiment on Lab 2."""
...
mkdir -p "$RESULTS_DIR"
printf '%s\n' "$RESULTS_DIR" > "$LAB_BASE/Results/LATEST_SHARP_AV2_MAMBA_ARTICLE80_RUN.txt"
...
# Real forward/backward smoke test before the 80-epoch run.
"$PYTHON_BIN" - <<'PY'
import torch
...
def main() -> None:
    ...
    experiment_root = base / "Codes" / f"SHARP_AV2_MAMBA_ARTICLE80_{stamp}"
    code_dir = experiment_root / "Code"
    results_root = base / "Results" / f"SHARP_AV2_MAMBA_ARTICLE80_{stamp}"
    ...
    (base / "Codes/LATEST_SHARP_AV2_MAMBA_ARTICLE80.txt").write_text(
        str(experiment_root) + "\n",
        encoding="utf-8",
    )
    (base / "Results/LATEST_SHARP_AV2_MAMBA_ARTICLE80.txt").write_text(
        str(results_root) + "\n",
        encoding="utf-8",
    )
