...
SETUP="$REPO/Lab 2/Codes/setup_lab2_temporal_agent_mamba_chunked80.py"
...
PATTERN='SHARP_AV2_(TEMPORAL_AGENT_MAMBA(8[0]|_CHUNKED8[0])|MAMBA_FUSED8[0])|run_(temporal_agent_mamba(_chunked)?_4gpu|av2_mamba_fused_4gpu_syncbn)[.]sh'
...
"$BASE/Codes/envs/sharp/bin/python" \
  setup_lab2_temporal_agent_mamba_chunked80.py

ROOT=$(cat "$BASE/Codes/LATEST_SHARP_AV2_TEMPORAL_AGENT_MAMBA_CHUNKED80.txt")
RESULTS=$(cat "$BASE/Results/LATEST_SHARP_AV2_TEMPORAL_AGENT_MAMBA_CHUNKED80.txt")
RUNNER="$ROOT/run_temporal_agent_mamba_chunked_4gpu.sh"
...
cd "$ROOT/Code"
echo "Starting stable chunked temporal-agent Mamba training in this foreground terminal."
bash "$RUNNER"
STATUS=$?
