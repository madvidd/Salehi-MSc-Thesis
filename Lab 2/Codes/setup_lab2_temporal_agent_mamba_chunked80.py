#!/usr/bin/env python3
"""Create the stable chunked SHARP temporal-agent Mamba AV2 experiment."""
...
class TemporalAgentMamba(nn.Module):
    ...
    Fused Mamba kernels receive fixed-size contiguous batches. Chunking along
    the agent dimension is mathematically independent and avoids shape-specific
    kernel faults without changing sequence semantics or learned parameters.
    """
...
echo "Running a blocking 32-batch preflight on real AV2 data."
...
echo "Real AV2 preflight passed. Starting the full four-GPU run."
...
gradient_clip_val=5
agent_chunk_size=128
fused_causal_conv1d=true
fused_selective_scan=true
real_data_preflight_batches=32
...
echo "SHARP_TEMPORAL_AGENT_MAMBA_CHUNKED_RUN_COMPLETE=True" | tee "$RESULTS_DIR/COMPLETED.txt"
...
def patch_temporal_mamba(code_dir: Path) -> None:
    ...
    train = replace_once(
        ...
        "    print(\n"
        "        'EXPERIMENT_VARIANT=temporal_agent_mamba_chunked MAMBA_ACTIVE=True '\n"
...
def main() -> None:
    ...
    experiment_root = base / "Codes" / f"SHARP_AV2_TEMPORAL_AGENT_MAMBA_CHUNKED80_{stamp}"
    results_root = base / "Results" / f"SHARP_AV2_TEMPORAL_AGENT_MAMBA_CHUNKED80_{stamp}"
    ...
    runner = experiment_root / "run_temporal_agent_mamba_chunked_4gpu.sh"
    write_lf(
        runner,
        RUNNER_TEMPLATE.format(
            ...
            code_dir=code_dir,
            results_root=results_root,
            variant="temporal_agent_mamba_chunked",
            pointer_name="LATEST_SHARP_AV2_TEMPORAL_AGENT_MAMBA_CHUNKED80_RUN.txt",
    ...
    manifest.write_text(
        "SHARP AV2 stable chunked temporal-agent Mamba experiment\n"
        ...
        "expand=1, dropout=0.1, per-channel LayerScale=0.01, fixed fused-kernel agent chunks of 128, contiguous CUDA inputs, and chronologically "
    ...
    (base / "Codes/LATEST_SHARP_AV2_TEMPORAL_AGENT_MAMBA_CHUNKED80.txt").write_text(
        str(experiment_root) + "\n",
        encoding="utf-8",
    )
    (base / "Results/LATEST_SHARP_AV2_TEMPORAL_AGENT_MAMBA_CHUNKED80.txt").write_text(
        str(results_root) + "\n",
        encoding="utf-8",
    )
