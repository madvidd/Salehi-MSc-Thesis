...
SETUP="$REPO/Lab 2/Codes/setup_lab2_temporal_agent_mamba_rotationfix80.py"
PREPARE="$REPO/Lab 2/Codes/prepare_lab2_nvidia_58015903_userspace.sh"
COMPAT_ROOT="$BASE/Codes/NVIDIA_USERSPACE_580.159.03"
COMPAT_LIB="$COMPAT_ROOT/runtime/lib"
COMPAT_BIN="$COMPAT_ROOT/runtime/bin"
PATTERN='SHARP_AV2_(TEMPORAL_AGENT_MAMBA(8[0]|_CHUNKED8[0]|_STABLECUDA8[0]|_ROTATIONFIX8[0])|MAMBA_FUSED8[0])|run_(temporal_agent_mamba(_chunked|_stablecuda|_rotationfix)?_4gpu|av2_mamba_fused_4gpu_syncbn)[.]sh'
...
OLD_FAILED_POINTER="$BASE/Results/LATEST_SHARP_AV2_TEMPORAL_AGENT_MAMBA_STABLECUDA80_RUN.txt"
...
"$BASE/Codes/envs/sharp/bin/python" \
  setup_lab2_temporal_agent_mamba_rotationfix80.py

ROOT=$(cat "$BASE/Codes/LATEST_SHARP_AV2_TEMPORAL_AGENT_MAMBA_ROTATIONFIX80.txt")
RESULTS=$(cat "$BASE/Results/LATEST_SHARP_AV2_TEMPORAL_AGENT_MAMBA_ROTATIONFIX80.txt")
RUNNER="$ROOT/run_temporal_agent_mamba_rotationfix_4gpu.sh"
...
cd "$ROOT/Code"
echo "Starting rotation-safe stable-CUDA temporal-agent Mamba training in this foreground terminal."
bash "$RUNNER"
STATUS=$?
