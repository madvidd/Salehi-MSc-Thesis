#!/usr/bin/env python3
"""Create the stable-CUDA SHARP temporal-agent Mamba AV2 experiment."""
...
echo "SHARP_TEMPORAL_AGENT_MAMBA_STABLECUDA_RUN_COMPLETE=True" | tee "$RESULTS_DIR/COMPLETED.txt"
...
def patch_temporal_mamba(code_dir: Path) -> None:
    ...
    train = replace_once(
        ...
        "    print(\n"
        "        'EXPERIMENT_VARIANT=temporal_agent_mamba_stablecuda MAMBA_ACTIVE=True '\n"
...
def main() -> None:
    ...
    experiment_root = base / "Codes" / f"SHARP_AV2_TEMPORAL_AGENT_MAMBA_STABLECUDA80_{stamp}"
    results_root = base / "Results" / f"SHARP_AV2_TEMPORAL_AGENT_MAMBA_STABLECUDA80_{stamp}"
    ...
    runner = experiment_root / "run_temporal_agent_mamba_stablecuda_4gpu.sh"
    write_lf(
        runner,
        RUNNER_TEMPLATE.format(
            ...
            code_dir=code_dir,
            results_root=results_root,
            variant="temporal_agent_mamba_stablecuda",
            pointer_name="LATEST_SHARP_AV2_TEMPORAL_AGENT_MAMBA_STABLECUDA80_RUN.txt",
    ...
    manifest.write_text(
        "SHARP AV2 stable-CUDA temporal-agent Mamba experiment\n"
    ...
    (base / "Codes/LATEST_SHARP_AV2_TEMPORAL_AGENT_MAMBA_STABLECUDA80.txt").write_text(
        str(experiment_root) + "\n",
        encoding="utf-8",
    )
    (base / "Results/LATEST_SHARP_AV2_TEMPORAL_AGENT_MAMBA_STABLECUDA80.txt").write_text(
        str(results_root) + "\n",
        encoding="utf-8",
    )
